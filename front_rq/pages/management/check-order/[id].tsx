...
const OrderCheck = () => {
  ...
  const { data: orderData } = useQuery(["orderData", orderId], () =>
    loadOrderAPI(Number(orderId))
  );
  ...
  useEffect(() => {
    if (
      orderData.order?.ProviderId !== myUserInfo.id &&
      ...
      <Container800>
        <Form>
          <Space size={8} wrap>
            ...
            ></Input>
            <Button type="primary" loading={loading} onClick={onConfirmOrder}>
              주문확인 완료
            </Button>
            <Popconfirm
              title="주문을 반려하시겠습니까?"
            ...
            <Link href={`/item/order/view/${orderId}`}>
              <a target="_blank">
...
export const getServerSideProps = async (
  ...
  const orderData = await loadOrderAPI(Number(orderId)).catch((error) => {
    return {
      redirect: {
        destination: "/unauth",
        permanent: false,
  ...
  if (
    orderData.order?.ProviderId !== response.id &&
  ...
  await queryClient.prefetchQuery(["orderData", orderId], () =>
    loadOrderAPI(Number(orderId))
  );
  return {
    props: {
