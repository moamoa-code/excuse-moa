...
const ItemEdit = ({ item, myUserInfo }) => {
  ...
  const { data: provider } = useQuery(["provider"], () =>
    loadProviderByIdAPI(Number(item.UserId))
  );
  const { Option } = Select;
  ...
  return (
    <>
      ...
      <Form>
        ...
        <Descriptions
          ...
          extra={
            item.UserId === myUserInfo.id ? (
              <>
                <Button onClick={() => Router.replace(`/management/items`)}>
                  목록으로 돌아가기
                </Button>
              </>
            ) : null
        ...
        >
          <Descriptions.Item label={<>제품 코드명</>}>
            <Input
              value={codeName}
        ...
        <Space size="middle" style={{ marginTop: "20px" }}>
          ...
          <Popconfirm
            ...
            cancelText="취소"
          >
            <Button loading={loading}>제품 삭제</Button>
          </Popconfirm>
        </Space>
      </Form>
      <br />
