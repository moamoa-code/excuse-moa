...
const UserProfile = () => {
  ...
  return (
    <Card>
      ...
      <br />
      <Space>
        <Button onClick={() => (router.replace(`/user/edit`))} loading={loading}>정보수정</Button>
        <Button onClick={() => (router.replace(`/user/addr-list`))} loading={loading}>주소록</Button>
        <Button onClick={onLogout} loading={loading} type="dashed" danger>로그아웃</Button>
      </Space>
